 ...
 * @date 2016.1.11
 */
public abstract class ScrollSelector<T> extends View {
    ...
    private void init(AttributeSet attrs) {
        if (attrs != null) {
            TypedArray typedArray = getContext().obtainStyledAttributes(attrs,
                    R.styleable.ScrollSelector);

            if (typedArray.hasValue(R.styleable.ScrollSelector_spv_center_item_background)) {
                setCenterItemBackground(typedArray.getDrawable(R.styleable.ScrollSelector_spv_center_item_background));
            }
            setVisibleItemCount(typedArray.getInt(
                    R.styleable.ScrollSelector_spv_visible_item_count,
                    getVisibleItemCount()));
            setCenterPosition(typedArray.getInt(
                    R.styleable.ScrollSelector_spv_center_item_position,
                    getCenterPosition()));
            setIsCirculation(typedArray.getBoolean(R.styleable.ScrollSelector_spv_is_circulation, isIsCirculation()));
            setDisallowInterceptTouch(typedArray.getBoolean(R.styleable.ScrollSelector_spv_disallow_intercept_touch, isDisallowInterceptTouch()));
            setHorizontal(typedArray.getInt(R.styleable.ScrollSelector_spv_orientation, mIsHorizontal ? 1 : 2) == 1);
            typedArray.recycle();
        }
    }
    ...
    private void notifySelected() {
        if (mListener != null) {
            ...
            post(new Runnable() {
                @Override
                public void run() {
                    mListener.onSelected(ScrollSelector.this, mSelected);
                }
            });
        }
    }
     ...
     * 自动滚动，默认速度为 0.6dp/ms
     *
     * @see ScrollSelector#autoScrollFast(int, long, float, Interpolator)
     */
    ...
    public interface OnSelectedListener {
        void onSelected(ScrollSelector scrollSelector, int position);
    }
